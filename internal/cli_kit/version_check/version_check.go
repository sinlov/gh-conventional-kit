...
func SemverVersionMinimumSupport(version string, lessVersion string) error {
	if version == "" {
		return errors.New("version is empty, please check")
	}

	if lessVersion == "" {
		return errors.New("lessVersion is empty, please check")
	}
	...
	checkVersion, errLessConstraint := semver.NewConstraint(">= " + lessVersion)
...
func SemverVersionConstraint(version string, minimumVersion, maximumVersion string) error {
	...
	if maximumVersion == "" {
		return errors.New("maximum version is empty, please check")
	}

	if minimumVersion == "" {
		return errors.New("minimum version is empty, please check")
	}
