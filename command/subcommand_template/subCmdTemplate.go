...
var maintainResourceActionFileList = []string{
...
func (n *TemplateCommand) Exec() error {
	...
	conventionalTitleEmbed, err := embed_source.GetResourceByLanguage(
		resource.GroupResource,
		path.Join(
			resource.DirNameContributingDoc,
			resource.KeyConventionalReadmeTitle,
		),
		constant.LangEnUS,
	)
	if err != nil {
	...
	resIssues, err := embed_source.GetResourceGroupByDir(
		resource.GroupResource,
		path.Join(resource.DirGithubAction, resource.KeyIssueTemplate),
	)
	if err != nil {
	...
	for _, maintainResourceFileItem := range maintainResourceActionFileList {
		...
		if errMaintainResEmbed != nil {
			slog.Errorf(
				errMaintainResEmbed,
				"GetResourceByLanguage for maintainResourceActionFileList %s",
				maintainResourceFileItem,
			)
	...
	for _, l := range n.LanguageSet {
		ccEmbedAsLang, errCCEmbedAsLang := embed_source.GetResourceListByLanguage(
			resource.GroupResourceAction,
