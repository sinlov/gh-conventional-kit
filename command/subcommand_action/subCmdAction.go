...
var (
	resourceActionDeployTagsFileList = []string{
...
func (n *ActionCommand) Exec() error {
	...
	if n.IsCiDeployTag {
		for _, maintainResourceFileItem := range resourceActionDeployTagsFileList {
			maintainResEmbedItem, errMaintainResEmbed := embed_source.GetResourceByLanguage(resource.GroupResourceActionWorkflowsDeployTags, maintainResourceFileItem, resourceLanguage)
			if errMaintainResEmbed != nil {
				slog.Errorf(errMaintainResEmbed, "GetResource for resourceActionDeployTagsFileList %s", maintainResourceFileItem)
				return cli_exit_urfave.ErrMsg(errMaintainResEmbed, "GetResource for resourceActionDeployTagsFileList")
			}
